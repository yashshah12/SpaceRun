...
import com.shesh.game.PowerUpManager;
...
public class Player extends GameObject {
	 ...
	 */
	private static final float LONG_JUMP_PRESS = 0.10f;
    ...
    private static final Color PLAYER_PARTICLES_COLOR = new Color(1, 1, 0.2f, 1);
	...
	public void update(float delta) {
		...
		if (Input.isKeyDown(Keys.UP) && velocity.y >= 0) {
			...
			if (state != State.JUMPING) {
				...
				Assets.soundJump.play(MainMenuScreen.ButtonListener.isMute ? 0 : 1);
			} else {
		...
		if (Input.isKeyDown(Keys.Z) && canShoot()) {
			...
			Assets.soundShoot.play(MainMenuScreen.ButtonListener.isMute ? 0 : 1);
		}
		...
		for (GameObject object : alGameObjects) {
			if (recBounds.overlaps(object.getBounds())) {
				...
				if (object.getClass().equals(Obstacle.class) || object.getClass().equals(Enemy.class) || object.getClass().equals(Bullet.class)) {
                    ...
                    level.addExplosion(new Explosion(position.cpy(), PLAYER_PARTICLES_COLOR, 40, 50));
