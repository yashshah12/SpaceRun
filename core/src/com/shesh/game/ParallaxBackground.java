...
public class ParallaxBackground {
	...
	public ParallaxBackground(ParallaxLayer... arLayers) {
		...
		for (int i = 0; i < arLayers.length; i++) {
			...
			arLayersCopy[i].getPosition().add(SpaceRun.WIDTH, 0);
		}
	}
	...
	public void update(float speed) {
		for (int i = 0; i < arLayers.length; i++) {
			...
			if (parallaxLayer.getPosition().x + SpaceRun.WIDTH < 0) {
				parallaxLayer.getPosition().add(2 * SpaceRun.WIDTH, 0);
			}

			if (parallaxLayerCopy.getPosition().x + SpaceRun.WIDTH < 0) {
				parallaxLayerCopy.getPosition().add(2 * SpaceRun.WIDTH, 0);
			}
		}
	}
	...
	public void render(SpriteBatch batch) {
		for(int i = 0; i < arLayers.length; i++){
			...
			batch.draw(parallaxLayer.getTexture(), parallaxLayer.getPosition().x, parallaxLayer.getPosition().y, SpaceRun.WIDTH, SpaceRun.HEIGHT);
			batch.draw(parallaxLayerCopy.getTexture(), parallaxLayerCopy.getPosition().x, parallaxLayerCopy.getPosition().y, SpaceRun.WIDTH, SpaceRun.HEIGHT);
		}
	}
}